...
function eXcell_co(cell){if (cell){this.cell=cell;this.grid=this.cell.parentNode.grid;this.combo=(this.cell._combo||this.grid.getCombo(this.cell._cellIndex));this.editable=true   
 ...
 this.obj.onkeyup=function(e){
	 var key=(e||event).keyCode;
		if (key==38 || key==40 || key==9|| key==13) return true;
	 var val = this.readonly ? String.fromCharCode((e||event).keyCode) : this.value;
	 var c = editor_obj.list.options;
	 $.each(c, function() {
				c.remove(this.key);
     });
	 var comboKeys = editor_obj.combo.getKeys();
	 for (var i = 0;i < comboKeys.length;i++)
	 {
	 	if(editor_obj.combo.get(comboKeys[i]).toLowerCase().indexOf(val.toLowerCase()) >= 0) 
			editor_obj.list.options[editor_obj.list.options.length]=new Option(editor_obj.combo.get(comboKeys[i]), comboKeys[i]);
	 }/*
	 var c = editor_obj.list.options;
				$.each(c, function() {
 ...
 if (comboKeys[i] == this.val){selOptId=this.list.options.length-1;fl=true}};if (fl == false){
	 this.list.options[this.list.options.length]=new Option(this.text, this.val === null ? "" : this.val);selOptId=this.list.options.length-1};document.body.appendChild(this.list)   
