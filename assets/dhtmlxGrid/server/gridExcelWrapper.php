...
class gridExcelWrapper {
    ...
    public function rowPrint($row, $cellColors, $rowHeight, $lineColor, $gridFontSize, $fontFamily) {
        ...
        for ($i = 0; $i < count($row); $i++) {
            $this->excel->setActiveSheetIndex(0);
			$text = $row[$i];
			$kieudulieu=strtolower($this->types[$i]);
			if ((isset($this->columns[0][$i]['type']))) {
				$kieudulieu=$this->columns[0][$i]['type'];
				if(($this->columns[0][$i]['type'] == 'ch')||($this->columns[0][$i]['type'] == 'ra'))
				{
					if ($text == '1') {
						$text = 'Yes';
					} else {
				...
				else if(($this->columns[0][$i]['type'] == 'nummeric')||($this->columns[0][$i]['type'] == 'num')||($this->columns[0][$i]['type'] == 'number')||($this->columns[0][$i]['type'] == 'numchu')||($this->columns[0][$i]['type'] == 'ron'))
					$text = trim(str_replace(",","",$text));//number_format(1*str_replace(",","",$text),2,".",",");
			}

			switch ($kieudulieu) {
				case 'string':
				case 'str':case 'ed':case 'ro':case 'edn':case 'edt':
					...
					break;
				case 'number':case 'ron':case 'nummeric':case 'price':case 'numchu':
				case 'num':
				...
				case 'boolean':
				case 'bool':
					$this->excel->getActiveSheet()->getCell($this->getColName($i).$this->currentRow)->setValueExplicit(trim($text), PHPExcel_Cell_DataType::TYPE_BOOL);
					break;
				case 'formula':
					$this->excel->getActiveSheet()->getCell($this->getColName($i).$this->currentRow)->setValueExplicit(trim($text), PHPExcel_Cell_DataType::TYPE_FORMULA);
					break;
				default:
